...
try {
  ...
  const args = live ? [values['declaration-live'] ? 'scripts/test-memory-declarations.ts' : 'scripts/test-memory-inputs.ts', '--env-file', resolve(values['env-file'])]
    : ['--test', '--test-concurrency=1', 'tests/integration/database.test.ts', 'tests/integration/inline.test.ts', 'tests/integration/intake.test.ts', 'tests/integration/declarations.test.ts', 'tests/integration/retrieval.test.ts', 'packages/cli/src/delivery.test.ts', 'packages/flow/src/flow.test.ts', 'packages/codex-hooks/src/flow.test.ts', 'tests/integration/runtime.test.ts']
