...
DSH SDK 运行专用提炼 Agent 和独立的关系比较 Agent，并保存各自的 DSH 会话。确定性代码负责接收、候选检索、状态、嵌入、事务和最终关系约束。Agent 不直接获得数据库或 Embedding 密钥，也不控制提交是否成功。v3 在原来源规则上增加原子事实、实体和时间字段；旧检查点仍按兼容契约读取，不重写已有正文和哈希。
...
新提炼按可独立更正的事实分条，保留使事实成立的限定条件和多条必要引用。`entities` 必须逐字出现在引用消息中；`valid_from/valid_until` 必须携带时间原文。程序负责引用校验，语义上的原子性仍由模型判断并以样例验证。
