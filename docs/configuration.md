...
CLI 入口是原生 `bin/jth.ts`，由 Bun 直接运行，npm 发布包保留 TypeScript 源码，不编译成 `.mjs`。手动通过 Bun 调用时使用 `bun -- bin/jth.ts ...`；`--` 保证 `--env-file` 由 JTH 解析。后台启动入口也使用这个分隔符。Hook 调用 PATH 中的 `jth -- ...`，避免 Vite+ 或 Bun 抢先解析 `--env-file`，也不记录安装目录或 Bun 的绝对路径；安装后可用 `command -v jth` 检查。新设备需安装 CLI，并在每个项目重新运行 `jth init` 生成本机配置。
