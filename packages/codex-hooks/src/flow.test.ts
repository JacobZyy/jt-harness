...
test('native flow hooks coexist with DSH capture, preserve other handlers, restore goals and never finish on Stop', async () => {
  ...
  try {
    ...
    store.install({ version: 1, workspace, envFile: config.envFile, projectIds: ['fixture'], businessIds: [], installedAt: new Date().toISOString() })
    ...
    const task = store.start({ goal: '解决原来的长任务目标偏移', acceptance: ['保留讨论边界'] }, {}, 'parent')
    ...
    assert.equal(store.binding('child')!.taskId, task.id)
    ...
    assert.equal(store.task(task.id).phase, 'discussion')
    ...
    assert.equal(store.task(task.id).goal, task.goal)
  } finally { store.close(); await rm(workspace, { recursive: true, force: true }) }
})

test('built CLI delivers a persistent task, real checks and completion while memory is offline', async () => {
  ...
  try {
    ...
    const recall = await execute(process.execPath, [resolve(root, 'bin/jth.mjs'), 'flow', 'recall', '--workspace', workspace, '--task', created.id, '--request', store.task(created.id).memory!.requestedAt]).catch(error => error)
