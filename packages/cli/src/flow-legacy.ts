...
/** Explicit compatibility entry for persisted Flow tasks; never called by native mode. */
export async function flowLegacyMain(root: string, args: string[]) {
  ...
  try {
    ...
    if (values.help || !command) { process.stdout.write(help.replaceAll('jth flow ', 'jth flow legacy ')); return }
    ...
    if (command === 'hook') {
      ...
      try {
        ...
        await startBackground(root, ['flow', 'legacy', 'sync', '--workspace', workspace], resolve(workspace, '.jth/flow-worker.log'))
        return
      }
      output(hook.output)
