...
import { z } from 'zod'
import { evaluatePolicy, findFlowWorkspace, planDraftSchema, planningTemplate, policyModeSchema, policyRequestSchema } from '@jt-harness/flow'
...
const invocationSchema = z.strictObject({ request: policyRequestSchema, plan: planDraftSchema.optional(), progress: codexProgressSchema.default([]) })
const help = `jth flow policy <file.json|-> [--mode adaptive|strict] [--host codex] [--plan-tool available|unavailable] [--role primary|delegate]
jth flow config [--scope user|project] [--mode adaptive|strict|inherit]
通用：--workspace <目录>
policy 读取主 Agent 显式声明的 request，可附 plan、progress；只返回决策、模板和待调用的原生工具参数，不保存或更新任务。
...
export async function flowPolicyMain(args: string[]) {
  try {
    const { values, positionals } = parseArgs({ args, allowPositionals: true, options: {
      ...
      'plan-tool': { type: 'string' }, role: { type: 'string' }, help: { type: 'boolean', short: 'h' },
    } })
    ...
    if (positionals[0] === 'config') {
      if (positionals.length !== 1 || values.host || values['plan-tool'] || values.role) throw new Error('config 参数不支持；运行 jth flow config --help')
