...
export async function startWorker(root: string, config: { dataDir: string, envFile: string }, legacy = false) {
  await mkdir(config.dataDir, { recursive: true, mode: 0o700 })
  const logPath = resolve(config.dataDir, 'worker.log')
  const log = await open(logPath, 'a', 0o600)
  try {
    await log.chmod(0o600)
    const child = spawn(process.execPath, [resolve(root, 'bin/jth.mjs'), 'memo', 'work', '--env-file', config.envFile, ...(legacy ? ['--legacy'] : [])], {
