...
export async function reconcileMemories(input: ComparisonInput, runtime: MemoryAgentOptions, context: AgentContext = {}) {
  ...
  const result = await runValidatedMemoryAgent(input, runtime, new URL('./reconcile.md', import.meta.url), reconciliationSchema, response => {
    const decision = reconciliationSchema.parse(JSON.parse(response))
