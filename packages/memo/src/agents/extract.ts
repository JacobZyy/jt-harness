...
import { assertAgentRun, runMemoryAgent } from './runtime.ts'
...
export async function extractMemories(input: unknown, runtime: MemoryAgentOptions, context: AgentContext = {}) {
  ...
  const result = await runMemoryAgent(submission, runtime, new URL('./agent.md', import.meta.url), agentExtractionSchema, context)
  agentExtractionSchema.parse(JSON.parse(result.response))
  return {
    status: 'extracted' as const, submission_id: submission.submission_id, source: submission.source, scope: submission.scope,
    run: result.run, ...parseExtraction(result.response, submission),
  }
}
