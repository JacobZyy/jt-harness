...
test('.env uses explicit paths and environment overrides; durable profile excludes credentials', async () => {
  ...
  try {
    ...
    assert.equal(config.agent.reasoningEffort, 'off')
    ...
    assert.equal((await loadConfig(root, envFile, {})).agent.reasoningEffort, 'high')
    assert.equal((await loadConfig(root, envFile, { JTH_DSH_REASONING_EFFORT: 'off' })).agent.reasoningEffort, 'off')
